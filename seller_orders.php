...
$sql = "SELECT SUM(price*quant) as order_total, order_items.order_number, price, quant, item_stat, buyer, order_status, first_name, last_name, order_time FROM order_items, myorder, userid WHERE order_items.seller_store = '$storename' AND order_items.order_number = myorder.order_number AND myorder.buyer = userid.uid AND order_items.item_stat != 2 AND order_items.item_stat != 0 AND order_items.item_stat != 3 GROUP BY myorder.order_number";
$coop_products =& $mdb2->query($sql);
while (($row = $coop_products->fetchRow(MDB2_FETCHMODE_ASSOC))) {
$order_pro[] = $row;
}
$smarty->assign('order_pro',$order_pro);
$sql2 = "SELECT SUM(price*quant) as member_total FROM order_items WHERE order_items.seller_store = '$storename' AND item_stat = 1";
...
$mdb2->disconnect();
$smarty->display('seller_order.tpl');
exit;
?>